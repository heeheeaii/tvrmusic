...
class AStarPathfinder( // A* 算法 Admissible Optimal
    ...
    fun findPath(start: Point, goal: Point): Pair<List<Point>, Double>? {
        ...
        if (start.layer >= numLayers || goal.layer >= numLayers ||
            start.row >= numRows || goal.row >= numRows ||
            start.col >= numCols || goal.col >= numCols ||
            start.layer < 0 || goal.layer < 0 ||
            start.row < 0 || goal.row < 0 ||
            start.col < 0 || goal.col < 0
        ) {
    ...
    fun printPathResult(
        result: Pair<List<AStarPathfinder.Point>, Double>?,
        start: AStarPathfinder.Point,
        goal: AStarPathfinder.Point
    ) {
        if (result != null) {
