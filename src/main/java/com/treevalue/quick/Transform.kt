...
class Transform {
    ...
    fun except(feeling: List<Position>, except: List<Position>) {
        ...
        val layerShape = feelingLayer.getShape()
        val (match, _) = matchPointsByMinCost(feeling, except, layerShape.width, layerShape.height)
        match.forEach { (inIdx, outIdx) ->
            GrowthManager.getInstance().requestGrowth(feelingLayer.getNeuron(feeling[inIdx]), except[outIdx])
        }
    }

    fun predicate(input: List<Position>): Pair<UUID, INDArray> {
//        MatchFinder.getInstance().matchPointsByMinCost()
//        SignalTransmissionManager.getInstance().submitEvent()
//        SignalEvent()// todo
    ...
    fun reinforce(except: List<Position>, predicated: INDArray, eventId: UUID) {
        TODO("Not yet implemented")
    }
